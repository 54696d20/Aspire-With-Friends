...
{
    Task<WeatherResponse?> GetCurrentWeatherAsync(string query);
    Task<WeatherResponse?> GetForecastAsync(string query, int days = 7);
    Task<List<SearchLocation>?> SearchLocationAsync(string query);
}
...
public class WeatherService : IWeatherService
{
    ...
    public async Task<WeatherResponse?> GetCurrentWeatherAsync(string query)
    {
        try
        {
            var response = await _httpClient.GetAsync($"myweather-api/api/weather/current/{Uri.EscapeDataString(query)}");
            ...
            _logger.LogWarning("Failed to get current weather: {StatusCode}", response.StatusCode);
            return null;
        }
        catch (Exception ex)
        {
    ...
    public async Task<WeatherResponse?> GetForecastAsync(string query, int days = 3)
    {
        try
        {
            var response = await _httpClient.GetAsync($"myweather-api/api/weather/forecast/{Uri.EscapeDataString(query)}?days={days}");
            if (response.IsSuccessStatusCode)
            {
                var json = await response.Content.ReadAsStringAsync();
                var options = new JsonSerializerOptions
            ...
            _logger.LogWarning("Failed to get forecast: {StatusCode}", response.StatusCode);
            return null;
        }
        catch (Exception ex)
        {
    ...
    public async Task<List<SearchLocation>?> SearchLocationAsync(string query)
    {
        try
        {
            var response = await _httpClient.GetAsync($"myweather-api/api/weather/search/{Uri.EscapeDataString(query)}");
            if (response.IsSuccessStatusCode)
            {
                var json = await response.Content.ReadAsStringAsync();
                var options = new JsonSerializerOptions
                ...
                return JsonSerializer.Deserialize<List<SearchLocation>>(json, options);
            }
            
            _logger.LogWarning("Failed to search locations: {StatusCode}", response.StatusCode);
            return null;
        }
        catch (Exception ex)
        {
...
// Weather API Response Models (same as WeatherAPI service)
